...
email['subject'] = 'fuck you asshole'

email.set_content(html.substitute(name = 'fuck'),'html') #this is the template class method with "substitute" the name in this code is from "index.html with $name" if you put $, it can be replaced by using template
...
with smtplib.SMTP_SSL(host = 'smtp.gmail.com', port = 465) as smtp: #had to use SMTP_SSL to encrypt the server
    ...
    smtp.login('sebasuta@gmail.com', '*********') #not the gmail login password but the app password generated in google account
